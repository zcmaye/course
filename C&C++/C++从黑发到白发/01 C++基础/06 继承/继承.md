...
![img](./assets/cpp-inheritance-2020-12-15-1.png)
...
C++的继承方式有三种，实际上最常使用的为`public`继承方式，而基类的成员访问限定符设置最多的为`public`和`protected`。

### 禁止继承

在C++中，可以使用`final`关键字来禁止类的继承。当一个类被声明为`final`时，任何尝试从该类继承的行为都会导致编译错误。
...
使用`final`关键字可以确保某些类或函数不会被进一步继承或重写，从而增加代码的安全性和稳定性。
