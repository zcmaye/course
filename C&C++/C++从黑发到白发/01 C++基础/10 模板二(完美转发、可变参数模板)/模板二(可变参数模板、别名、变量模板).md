...
可变参数模板对模板参数进行了高度泛化，能表示0到任意个数、任意类型的参数，这为C++编程带来了更大的灵活性和表达力。

## 定义与使用

可变参数模板通过在模板参数列表中使用省略号`...`来定义。这允许模板接受零个或多个模板参数。

例如，一个可变参数的函数模板可以这样定义：

```cpp
template<typename ...Args>
void foo(Args ...args)
{
    std::println("{} args count：", __FUNCSIG__,sizeof...(args));
}
```

在上面的代码中`class ...Args`是类型模板形参包，他可以接受零个或者多个类型的模板实参。`Args ...args`叫做函数形参包，它出现在函数的形参列表中，可以接受零个或者多个函数实参。`sizeof...(args)` 其中`sizeof...`是C++的运算符，专门用来获取形参包的**参数个数**。
...
由于不能直接访问参数包中的每个参数，你需要使用参数包**展开的技术**来处理参数包中的参数。只有结合了包展开，才能发挥变参模板的能力。

### 递归展开

递归展开是通过递归函数调用来逐个处理参数包中的参数。为了终止递归，你需要重载函数模板以处理最后一个参数或无参数的情况。
...
int main()
{
     std::println("{}", sum(1, 5.0, 11.7));
}
```

### 逗号表达式展开

逗号表达式展开是通过在函数体中使用逗号表达式来展开参数包。逗号表达式从左到右计算各个表达式，并将最后一个表达式的值作为返回值。例如：
...
int main()
{
	std::println("{}",sum(std::string("hello "), "C++", "Maye"));
}
```
...
void test_args()
{
	std::println("{}",visit<0>(1, 3, 5, 7, 9, "ehllo"));
	std::println("{}",visit<1>(1, 3, 5, 7, 9, "ehllo"));
	std::println("{}",visit<2>(1, 3, 5, 7, 9, "ehllo"));
	std::println("{}",visit<3>(1, 3, 5, 7, 9, "ehllo"));
	std::println("{}",visit<4>(1, 3, 5, 7, 9, "ehllo"));
	std::println("{}",visit<5>(1, 3, 5, 7, 9, "ehllo"));
}
```
