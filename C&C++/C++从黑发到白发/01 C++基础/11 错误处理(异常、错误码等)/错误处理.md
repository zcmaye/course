...
{
	auto _fmt = std::format_to("{}:{}:({}) -> {} ", location.file_name(), location.function_name(), location.line(),fmt);
    std::string result;
	std::vformat_to(std::back_inserter(result), _fmt, std::make_format_args(args...));
	std::print("{}", result);
}
```
