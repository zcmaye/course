...
![addd5331-14a9-4a74-bf62-0b974b2b3620](./assets/addd5331-14a9-4a74-bf62-0b974b2b3620.jpg)
...
// 定义每个字符的图案，这里每个字符由7行组成，每行最多5个字符
const char patterns[52 + 2][7][6] = {
    // 大写字母
    {
        "  *  ",
        " * * ",
...
void print_char(char c) {
    int index = get_pattern_index(c);
    if (index == -1) {
        return;
    }
    for (int i = 0; i < 7; i++) {
        printf("%s ", patterns[index][i]);
        printf("\n");
    }
}
...
void print_string(const char* str) {
    for (int i = 0; i < 7; i++) {
        for (int j = 0; str[j] != '\0'; j++) {
            int index = get_pattern_index(str[j]);
            if (index != -1) {
                printf("%s ", patterns[index][i]);
            }
        }
