...
[[用户自定义变量](https://www.cnblogs.com/wsx2019/p/15769259.html)](https://www.cnblogs.com/wsx2019/p/15769259.html)
...
**范例2：**声明存储过程“update_sal_by_eno”，定义IN参数eno，输入员工编号。判断该员工 薪资如果低于3000元并且入职时间超过5年，就涨薪500元；否则就不变。
...
BEGIN
	...
	IF emp_sal < 3000 AND hire_year > 5
	THEN UPDATE emps SET sal = sal + 500 WHERE emono = eno; 
	END IF;
END ;
```

**范例3：**声明存储过程“update_sal_by_eno2”，定义IN参数eno，输入员工编号。判断该员工  薪资如果低于300元并且入职时间超过5年，就涨薪500元；否则就涨薪100元。
...
**范例4：**声明存储过程“update_sal_by_eno3”，定义IN参数eno，输入员工编号。判断该员工  薪资如果低于4000元，就更新薪资为4000元；薪资如果大于等于4000元且低于10000的，但是奖金 比例为NULL的，就更新奖金比例为0.01；其他的涨薪100元。

```mysql
CREATE PROCEDURE update_sal_by_eno3(IN eno INT) 
BEGIN
	DECLARE emp_sal DOUBLE; 
	DECLARE bonus DECIMAL(3,2);

	SELECT sal INTO emp_sal FROM emps WHERE empno = eno; 
	SELECT comm INTO bonus FROM emps WHERE empno = eno;

	IF emp_sal < 9000
		THEN UPDATE emps SET sal = 9000 WHERE empno = eno;
	ELSEIF emp_sal < 10000 AND bonus IS NULL
		THEN UPDATE emps SET comm = 0.01 WHERE empno = eno;
	ELSE
		UPDATE emps SET sal = sal + 100 WHERE empno = eno; 
	END IF;
END //
...
CASE 语句的语法结构1：
 ...
 ```mysql
#情况一：类似于switch CASE 表达式
WHEN 值1 THEN 结果1或语句1(如果是语句，需要加分号) WHEN 值2 THEN 结果2或语句2(如果是语句，需要加分号)
...
...
**范例3：**声明存储过程“update_salary_by_eno4”，定义IN参数eno，输入员工编号。判断该员工  薪资如果低于9000元，就更新薪资为9000元；薪资大于等于9000元且低于10000的，但是奖金比例 为NULL的，就更新奖金比例为0.01；其他的涨薪100元。

```mysql
CREATE PROCEDURE update_salary_by_eno4(IN eno INT) 
BEGIN
	DECLARE emp_sal DOUBLE; DECLARE bonus DECIMAL(3,2);
...
**范例4：**声明存储过程update_salary_by_eno5，定义IN参数emp_id，输入员工编号。判断该员工的   入职年限，如果是0年，薪资涨50；如果是1年，薪资涨100；如果是2年，薪资涨200；如果是3年， 薪资涨300；如果是4年，薪资涨400；其他的涨薪500。

```mysql
CREATE PROCEDURE update_salary_by_eid5(IN eno INT) 
BEGIN
	DECLARE emp_sal DOUBLE; 
	DECLARE hire_year DOUBLE;
	...
	SELECT DATEDIFF(CURDATE(),hire_date)/365 INTO hire_year FROM emps WHERE empno = eno;

	CASE hire_year
		WHEN 0 
			THEN UPDATE emps SET sal=sal+50 WHERE empno = eno; 			
		WHEN 1 
			THEN UPDATE emps SET sal=sal+100 WHERE empno = eno; 		
		WHEN 2 	
			THEN UPDATE emps SET sal=sal+200 WHERE empno = eno; 	
		WHEN 3 
			THEN UPDATE emps SET sal=sal+300 WHERE empno = eno; 
		WHEN 4 
			THEN UPDATE emps SET sal=sal+400 WHERE empno = eno; 		
		ELSE 
			UPDATE emps SET sal=sal+500 WHERE empno = eno;
	END CASE;
END //
```
...
**范例2：**当市场环境变好时，公司为了奖励大家，决定给大家涨工资。声明存储过程“update_sal_loop()”，声明OUT参数num，输出循环次数。存储过程中实现循环给大家涨薪，薪资涨为   原来的1.1倍。直到全公司的平均薪资达到12000结束。并统计循环次数。
...
**范例2：**市场环境不好时，公司为了渡过难关，决定暂时降低大家的薪资。声明存储过程“update_salary_while()”，声明OUT参数num，输出循环次数。存储过程中实现循环给大家降薪，薪资降   为原来的90%。直到全公司的平均薪资达到5000结束。并统计循环次数。
...
**范例2：**当市场环境变好时，公司为了奖励大家，决定给大家涨工资。声明存储过程“update_salary_repeat()”，声明OUT参数num，输出循环次数。存储过程中实现循环给大家涨薪，薪资涨   为原来的1.15倍。直到全公司的平均薪资达到13000结束。并统计循环次数。
...
BEGIN
	...
	#定义游标
	DECLARE emp_cursor CURSOR FOR SELECT sal FROM emps ORDER BY sal DESC; #打开游标
	OPEN emp_cursor;
