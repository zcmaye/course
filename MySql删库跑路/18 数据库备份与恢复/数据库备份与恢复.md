...
**举例：**使用SELECT…INTO OUTFILE将test数据库中emp表导出到文本文件。 
...
**举例**：使用mysqldump命令将将test数据库中emp表中的记录导出到文本文件：
...
**举例**2：使用mysqldump将test数据库中的emp表导出到文本文件，使用FIELDS选项，要求字段之   间使用逗号“，”间隔，所有字符类型字段值用双引号括起来：
...
**举例**1：使用mysql语句导出test数据中emp表中的记录到文本文件：
...
```mysql
mysql -uroot -p --vertical --execute="SELECT * FROM emp;" test > "C:/Users/Maye/backup/emp_1.txt"
```
...
从文本文件account.txt中恢复数据：
 ...
 ```mysql
LOAD DATA INFILE 'C:/Users/Maye/backup/emp_0.txt' INTO TABLE test.emp;
 ```
...
**举例：**导出文件emp.txt，字段之间使用逗号"，"间隔，字段值用双引号括起来：
...
```mysql
mysqlimport -uroot -p test 'C:/Users/Maye/backup/emp_3.csv' --fields-terminated-by=',' --fields-optionally-enclosed-by='\"'
```
