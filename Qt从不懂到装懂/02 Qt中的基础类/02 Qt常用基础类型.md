...
Qt基本数据类型定义在`#include <qtypes.h>` 中，但一般无需单独包含头文件，在其他头文件中会自动包含，Qt基本数据类型有：
...
| qreal      | double 或 float                              | 默认为double，可以通过添加预编译宏修改<br>`add_compile_definitions(-DQT_COORD_TYPE=float)` |
...
| qsizetype  | size_t                                       | 去掉了size_t的符号，也就是有符号的类型                       |

# 2. 日志输出

编写程序设计与开发日志是软件开发过程中不可或缺的一部分。它不仅有助于记录开发过程，还能为后续的调试、维护和优化提供重要依据。以下是编写高质量开发日志的关键点：

## 日志的目的

日志的主要目的是记录程序运行的关键信息，帮助开发者快速定位问题。它类似于系统的“黑匣子”，在出现异常时提供详细的上下文信息。日志的作用包括：

- **监控与诊断**：快速定位系统故障，减少排查时间。
- **审计与安全**：记录用户行为，便于安全审计。
- **性能分析**：通过日志数据优化系统性能。
- **业务跟踪**：确保业务逻辑的正确性。
...
- **时间戳**：记录日志生成的时间，便于追踪问题。
- **日志级别**：如 DEBUG、INFO、WARNING、ERROR 等，表示日志的重要性。
- **日志内容**：描述具体的操作、异常信息或业务逻辑。
- **来源信息**：包括模块名、方法名、用户 ID 等，便于定位问题。
- **上下文信息**：如请求 ID、会话 ID、IP 地址等，用于关联和分析。

## 日志级别

在Qt中进行日志输出, 一般不使用c中的`printf`, 也不是使用C++中的`cout`, Qt框架提供了专门用于日志输出的类, 头文件名为 `QDebug`。

Qt提供了多种方式进行日志输出，常用的如下：
...
qFatal("hello %s","qFatal");		//打印日志后，会直接中断程序
```
...
qWarning()<<"qWarnning";
qCritical()<<"qCritical";
qFatal()<<"qFatal";					//打印日志后，会直接中断程序
```

> 输出日志后会自动换行，而且连续输出多个数据，会自动加上空格。

## 日志格式化

默认情况下，日志格式是只输出对应的日志内容没有额外信息的。我们可以通过修改环境变量`QT_MESSAGE_PATTERN`或者调用方法`qSetMessagePattern`来修改日志的输出格式，如果同时设置环境变量`QT_MESSAGE_PATTERN`优先。日志格式中常用的占位符号如下所示:

| 格式             | 描述                                                         |
| ---------------- | ------------------------------------------------------------ |
| %{appname}       | 应用程序的名称(QCoreApplication::applicationName())          |
| %{category}      | 日志类别(默认为default)，自定义消息日志可以修改<br>QMessageLogger(QT_MESSAGELOG_FILE, QT_MESSAGELOG_LINE, QT_MESSAGELOG_FUNC,"custom").debug("custom critical"); |
| %{file}          | 源文件路径                                                   |
| %{function}      | 函数名                                                       |
| %{line}          | 行号                                                         |
| %{message}       | 实际的日志消息                                               |
| %{pid}           | 进程ID(QCoreApplication::applicationPid())                   |
| %{threadid}      | 线程ID                                                       |
| %{qthreadptr}    | 指向当前 QThread 的指针（即 QThread::currentThread() 的结果） |
| %{type}          | 日志级别("debug", "warning", "critical" or "fatal")          |
| %{time process}  | 消息发送的时间（以秒为单位，自进程启动以来的时长）（其中“process”一词为原意） |
| %{time boot}     | 该消息的时间（以自系统启动以来的秒数表示，如果能够确定的话（“boot”这个标记是原样保留的）。 |
| %{time [format]} | 记录消息发生时的系统时间，通过将格式传递给 QDateTime::toString() 函数来进行格式化。<br>若未指定格式，则使用 Qt::ISODate 格式。 |

还可以通过在消息类型上使用条件语句 `%{if-debug}`、 `%{if-info}`、 `%{if-warning}`、 `%{if-critical}` 或 `%{if-fatal}` 来实现这一功能，之后跟上 `%{endif}` 符号。 `%{if-*}` 和 `%{endif}` 之间的内容只有在类型匹配时才会被打印出来。

最后， `%{if-category}` ... `%{endif}` 之间的文本只有在类别不是默认类别时才会被打印出来。

+ 打印所有日志格式

```cpp
qSetMessagePattern("[%{appname} %{category} %{file} %{function} %{line} %{pid} %{threadid} %{qthreadptr} %{function} %{time process} %{time boot} %{time yyyy-MM-dd}] %{message}");
```
...
```cpp
// -- 当消息类别不是默认类别时，打印程序名称
qSetMessagePattern("[%{if-category}%{appname}%{endif} %{file} %{function} %{line} ] %{message}");
qDebug() << "好帅";		//不输出名称
QMessageLogger(QT_MESSAGELOG_FILE, QT_MESSAGELOG_LINE, QT_MESSAGELOG_FUNC,"custom").debug("custom critical");	//输出名称
```

+ 环境变量，除了可以直接在环境变量中设置外，还可以直接在代码中进行设置

```cpp
qputenv("QT_MESSAGE_PATTERN", QByteArray("[%{appname} %{category} %{file} %{function} %{line} %{pid} %{threadid} %{qthreadptr} %{function} %{time process} %{time boot} %{time yyyy-MM-dd}] %{message}"));
```

## 输出自定义类型

在编写程序过程中，我们肯定有很多自定义类型，但是默认情况下qt的日志库是不支持输出自定义对象的。这时候我们可以通过重写操作符实现对自定义象的日志输出。使用方法如下:
...
消息处理程序是一个函数，用于从 Qt 的日志基础设施中打印出调试、信息、警告、严重和致命消息。默认情况下，Qt 使用一个标准的消息处理程序，该程序会根据操作系统和 Qt 配置的不同将消息格式化并输出到相应的日志目标中。安装自己的消息处理程序可以让您完全掌控情况，例如将消息记录到文件系统中。

通过`qInstallMessageHandler`函数安装消息处理程序：
...
`QString` 和 `QByteArray` 是 Qt 中最常用的两个数据容器类。简单来说，**`QString` 用来处理文本，`QByteArray` 用来处理原始字节数据**。

QByteArray相当于C++中的std::string，对于存储的数据没有进行任何格式处理，主要用来存储原始二进制数据或未知编码的8位数据；以原始的8位字节（`char`）存储。

QString专门用来处理 **Unicode 文本**，是处理字符串的首选；以 UTF-16 编码存储，每个字符是一个16位的 `QChar`。

## 理解区别

### 本质不同：文本 vs. 字节

这是最根本的区别。`QString` 理解字符，而 `QByteArray` 不理解。

- **`QString`**：它的存在就是为了**表示文本**。它内部使用 Unicode（UTF-16）编码，这意味着无论中文、英文还是阿拉伯文，它都能统一处理，并且能正确地进行大小写转换、比较等操作。
- **`QByteArray`**：它只是一个**字节的容器**。它不关心这些字节代表什么——可能是一个文本文件的内容，也可能是一张图片的数据，或者是一个网络包。它不会将这些字节解释为“字符”。

### 编码意识

这是它们最大的行为差异。

- **`QString` 有编码意识**：当你从 `QByteArray` 构建一个 `QString` 时，你必须告诉它这些字节是何种编码（UTF-8, Latin-1, 系统本地编码等）。如果告诉它的编码和实际数据不一致，就会出现**乱码**。
- **`QByteArray` 无编码意识**：它只是存储字节，不进行任何编码转换。例如，它对文本的操作仅限于 `toUpper()` 这样的 ASCII 操作。

### 典型应用场景

- **什么时候用 `QString`？**
  只要你处理的是**文本**，比如用户在界面上输入的名字、从数据库读取的标题、需要显示在 `QLabel` 上的信息，都应该使用 `QString`。
- **什么时候用 `QByteArray`？**
  - **网络通信**：从 `QTcpSocket` 读取或写入的数据，最初都是以 `QByteArray` 形式存在的。
  - **文件 I/O**：`QFile` 的 `readAll()` 函数返回的就是 `QByteArray`，因为它不知道文件里存的是文本还是二进制数据。
  - **数据编码**：比如将数据转换成 Base64，或者从 Base64 解码回来，这些操作都是基于 `QByteArray` 的。
...
QString提供的一种格式化字符串输出的函数arg()，非常方便。
...
`QVariant` 是 Qt 中一个非常有特色的**万能数据类型**。它就像一个有魔力的容器，几乎可以存放 Qt 中任何常见的数据类型。

`QVariant` 可以理解为一种**联合体（union）的增强版**。它可以在运行时动态地存储和转换各种数据类型，让不同类型的值可以像同一种类型那样被传递和使用。
...
QElapsedTimer 类通常用于快速计算两个事件之间所经过的时间。

该类的典型应用场景是用于计算一段缓慢操作所耗费的时间。此类场景最简单的例子是用于调试目的，如下例所示：
...
在这个示例中，定时器是通过调用“start()”函数来启动的，而所经过的时间则是通过“elapsed()”函数来计算的。

## 6.5 QDeadlineTimer

QDeadlineTimer 类通常用于计算未来的截止日期，并检查截止日期是否已过期。QDeadlineTimer 还可用于无截止日期的情况（即“永不过期”）。它与 QElapsedTimer 形成互补关系，后者用于计算自调用 QElapsedTimer:：start() 以来已经过去了多少时间。

QDeadlineTimer 提供的 API 比 QElapsedTimer::hasExpired() 更为便捷。

该类的典型应用场景是在相关操作开始之前创建一个 QDeadlineTimer，然后通过调用 remainingTime() 或 hasExpired() 方法来判断是否继续尝试该操作。可以将 QDeadlineTimer 对象传递给要执行此操作的函数，以便这些函数了解还需要运行多长时间。



许多 QDeadlineTimer 函数都涉及超时值，这些值均以毫秒为单位进行测量。有两个特殊的值，与许多其他名为“等待”或类似名称的 Qt 函数的值相同：

+ 0：时间已用尽，已过期
+ -1：剩余时间无限，计时器永不失效

```cpp
QDeadlineTimer deadline(10000);  // 10秒
...
// 获取剩余时间
qint64 remaining = deadline.remainingTime();  // 毫秒
if (remaining == 0) {
    qDebug() << "刚刚过期";
} else if (remaining == -1) {
    qDebug() << "无限期（永不过期）";
} else {
    qDebug() << "剩余" << remaining << "毫秒";
}
```
...
```cpp
bool qIsInf(double d);			//是不是无穷大的浮点数；常量INFINITY
bool qIsNaN(double d);			//是不是无效浮点数，例如负数求平方根等，像这类情况，获取到的浮点数的值是无效的。常量NAN
bool qIsFinite(double d);		//是不是有限的浮点数；qIsInf的相反判断
int qFpClassify(double val);	//对浮点数值进行分类。返回值是以下之一：
```

+ FP_NAN 无效数值
+ FP_INFINITE 无穷大（正无穷或负无穷）
+ FP_ZERO zero（正数或负数）
+ FP_NORMAL（浮点数格式）：具有完整尾数的有限值
+ FP_SUBNORMAL 有限值，其尾数较小
...
//限制一个值在一个指定的最小值和最大值之间,本质上，它的实现是基于 qMin() 和 qMax() 的组合qMin(qMax(val, min), max)。
