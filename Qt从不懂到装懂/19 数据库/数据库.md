...
可以通过静态成员`drivers()`查看支持的所有驱动。

```cpp
qInfo() << QSqlDatabase::drivers();
```

> 默认是不支持MySql驱动的，需要自己编译，后续再说 
...
如果你想要查看当前数据库的所有表，可以使用` QStringList tables() const`函数获取。
...
知道了当前数据库有哪些表之后，就可以使用`QSqlRecord record(const QString &tablename) const`查看指定表的字段信息。
...
调用setRelation()函数建立两个表之间的关系。 调用指定表student中的第1列是与表course的字段cno映射的外键，并且视图应该向用户显示城市的名称字段。 (把student表中的指定列的(cno)字段，换成与course表中cno对应的记录的cname字段)
...
```css
E:\Mysoftware\Qt\6.10.2\Src\qtbase\src\plugins\sqldrivers
```

![image-20260319192337820](./assets/image-20260319192337820.png)
