...
MySQL 提供了丰富的内置函数，涵盖**字符串处理、数值运算、日期时间操作、聚合计算**及**控制流**等，能显著提升数据处理与分析效率。
...
在MySQL中，函数是用于简化操作、提高查询能力的重要工具。根据作用范围，函数可以分为**单行函数**和**多行函数**。

**单行函数**：单行函数对每一行数据分别进行操作，并返回一个结果。

**多行函数**：多行函数（聚合函数）对一组数据进行计算，返回一个结果，常与`GROUP BY`结合使用。 
