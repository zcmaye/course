...
变量由系统定义，不是用户定义，属于服务器层面。启动MySQL服务，生成MySQL服务实例期间， MySQL将为MySQL服务器内存中的系统变量赋值，这些系统变量定义了当前MySQL服务实例的属性、特征。这些系统变量的值要么是编译MySQL时参数的默认值，要么是 配置文件（例如my.ini等）中的参数值。大家可以在官网[Server System Variables](https://dev.mysql.com/doc/refman/8.0/en/server-system-variables.html)中查看MySQL文档的系统变量。
...
可以使用如下命令查看 MySQL 服务器端程序支持的系统变量及它们的当前值。
...
作为 MySQL 编码规范，MySQL 中的系统变量以两个`@`开头，其中`@@global`仅用于标记全局系统变量，`@@session`仅用于标记会话系统变量。`@@`首先表示会话系统变量，如果会话系统变量不存在，则表示全局系统变量。
...
+ 查看指定会话变量的值
  ...
  也可以使用简化的写法，不指定`global`或`session`默认就是`session`会话变量：
...
大部分系统变量都可以通过启动服务器时传送启动选项的方式来进行设置。前面提到，添加启动选项主要有两种方式，分别是通过命令行添加启动选项和通过配置文件添加启动选项。

通过配置文件添加启动选项。例如，编辑 my.ini 配置文件，如下所示：
...
表示 MySQL 启动时允许同时连接到服务器的客户端数量为 10 个。

添加配置文件之后，需要重启一下`MySQL`服务， 启动服务器端程序后，我们再来查看一下系统变量的值，如下所示：
...
对于大部分系统变量来说，它们的值可以在服务器端程序运行过程中进行动态修改而无须停止并重启服务器。但是，系统变量有作用范围之分。

在MySQL服务运行期间，使用`SET`语句重新设置系统变量的值，SET语句语法如下：

```sql
SET [GLOBAL|LOCAL] var_name = 值;
```
...
注意`max_connections`是全局变量，不是会话变量，所以不能通过会话变量进行设置：

```sql
SET SESSION max_connections = 10;
```

报错：`1229 - Variable 'max_connections' is a GLOBAL variable and should be set with SET GLOBAL`

我们可以设置客户端字符集，来测试设置会话变量：
...
MySQL会将该命令的配置保存到数据目录下的 `my.ini` 文件中，下次启动时会读取该文件，用其中的配置来覆盖默认的配置文件。
...
局部变量是一种只能在存储过程或函数的`BEGIN...END`代码块中声明和使用的变量。局部变量的作用范围仅限于其所在的代码块，代码块执行完毕后，局部变量就会消失。

局部变量只能放在 BEGIN ... END 中，而且**只能放在第一句**。
...
**范例2：**声明存储过程“proc_update_sal_by_eno”，定义IN参数eno，输入员工编号。判断该员工 薪资如果低于5000元并且入职时间超过5年，就涨薪500元；否则就不变。
...
**范例3：**声明存储过程“proc_update_sal_by_eno2”，定义IN参数eno，输入员工编号。判断该员工  薪资如果低于6000元，就更新薪资为6000元；薪资如果大于等于6000元且低于8000的，但是奖金 比例为NULL的，就更新奖金比例为0.01；其他的涨薪100元。
...
说明：定义条件和处理程序必须定义在**存储过程**或**函数**中。

### 3.1 案例分析

对于emp表，给`sal`字段先加上`NOT NULL`约束：

```sql
ALTER TABLE emp MODIFY sal DECIMAL(7,2) NOT NULL;
```

然后创建一个名称为`proc_update_data`的存储过程。代码如下： 
...
```mysql
DECLARE condition_name CONDITION FOR [error_code| SQLSTATE sqlstate_value]
```

**其中：**

+ **condition_name：**表示定义的条件的名称，合法的标识符即可；
+ **error_code：**表示数值类型错误代码
+ **sqlstate_value：**表示长度为5的字符串类型错误代码。

例如，在ERROR 1048 (23000)中，1048是`error_code`，'23000'是`sqlstate_value`。

例如，在ERROR 1146（42S02）中，1146是`error_code`，'42S02'是`sqlstate_value`。
...
+ **handler_type：**是处理错误的类型，处理方式有3个取值：CONTINUE、EXIT、UNDO：
...
+ **condition_value：**表示错误类型（即条件）可以有如下取值：
...
+ **sp_statement：**参数为程序语句段，表示在遇到定义的错误时需要执行的存储过程或函数。如果出现上述条件之一，则采用对应的处理方式，并执行指定的处理语句。语句可以是像`SET 变量 = 值`这样的简单语句，也可以是使用 BEGIN ... END 编写的复合语句。
...
在存储过程中，我们可以使用SIGNAL语句抛出异常。SIGNAL语句有两个主要参数：SQLSTATE（一个5位的字符串，用于标识异常的类型）和MESSAGE（异常的消息）。

```sql
SIGNAL SQLSTATE sqlstate_value SET  MESSAGE_TEXT = 'exception_message';
```

为了演示抛出异常的过程，我们在存储过程中添加了一个简单的判断条件，并在条件不满足时抛出异常。

```sql
CREATE PROCEDURE `proc_test`()
BEGIN
    DECLARE custom_exception CONDITION FOR 'This is a custom exception.';
    DECLARE i INT;
    
    SET i = 10;
    
    IF i > 5 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'i should be less than or equal to 5.';
    END IF;
END;
```
...
比如，我们查询了 emps数据表中工资高于4500的员工都有哪些：

```mysql
SELECT empno,ename,sal FROM emps WHERE sal > 4500;
```
