...
| **select_type**   | 查询类型            | 1. **SIMPLE**：简单 SELECT（不含子查询或 UNION）<br/> 2. **PRIMARY**：最外层查询 <br/> 3. **SUBQUERY**：子查询<br/> 4. **DERIVED**：派生表（FROM 子句中的子查询）<br/> 5. **UNION**：UNION 中的第二个或后续查询 <br/> 6. **UNION RESULT**：UNION 的结果<br> 7. **MATERIALIZED**：查询被物化，即结果被存储到临时表中 |
...
filtered字段值不是越小越好，具体情况具体分析：

#####  `filtered` 字段的含义

- **定义**：MySQL 预估在 `type` 访问方式后，经过 WHERE 条件过滤，**剩余的记录百分比**
- **计算公式**：`filtered = (过滤后行数 / 扫描行数) × 100%`
- **范围**：0.00% ~ 100.00%
- **精度**：MySQL 8.0 之前是整数，8.0+ 支持小数

##### 不同场景分析

| filtered 值 | 含义                                | 是否理想               |
| :---------- | :---------------------------------- | :--------------------- |
| **100%**    | 没有 WHERE 条件，或所有行都满足条件 | 中性：看业务需求       |
| **50%**     | 一半行被过滤掉                      | **通常较好**：索引有效 |
| **10%**     | 只保留 10% 的行                     | **通常很好**：高效过滤 |
| **1%**      | 只保留 1% 的行                      | **极好**：高度选择性   |
| **0%**      | 无匹配行（早期停止）                | **非常好**：快速返回空 |

##### 关键原则：综合评估，不能只看filtered

| 组合场景                         | 分析                 | 优化建议         |
| :------------------------------- | :------------------- | :--------------- |
| **type = ALL + filtered 小**     | 全表扫描但过滤率高   | 考虑添加索引     |
| **type = ref + filtered 小**     | 索引有效过滤         | 理想情况         |
| **type = index + filtered 大**   | 全索引扫描但过滤率低 | 可能不需要该索引 |
| **type = range + filtered 极小** | 范围扫描高度过滤     | 最佳情况之一     |
...
```sql
mysql> EXPLAIN SELECT * FROM emp WHERE ename = 'maye';
+----+-------------+-------+------------+------+---------------+-----------+---------+-------+------+----------+-------+
| id | select_type | table | partitions | type | possible_keys | key       | key_len | ref   | rows | filtered | Extra |
+----+-------------+-------+------------+------+---------------+-----------+---------+-------+------+----------+-------+
|  1 | SIMPLE      | emp   | NULL       | ref  | idx_ename     | idx_ename | 43      | const |    1 |   100.00 | NULL  |
+----+-------------+-------+------------+------+---------------+-----------+---------+-------+------+----------+-------+
1 row in set, 1 warning (0.00 sec)
```
...
*************************** 1. row ***************************
  Level: Note
   Code: 1003
Message: /* select#1 */ select `hdy`.`emp`.`empno` AS `empno`,`hdy`.`emp`.`ename` AS `ename`,`hdy`.`emp`.`job` AS `job`,`hdy`.`emp`.`mgr` AS `mgr`,`hdy`.`emp`.`hiredate` AS `hiredate`,`hdy`.`emp`.`sal` AS `sal`,`hdy`.`emp`.`comm` AS `comm`,`hdy`.`emp`.`deptno` AS `deptno`,`hdy`.`emp`.`dname` AS `dname` from `hdy`.`emp` where ((`hdy`.`emp`.`empno` > 7788) and (`hdy`.`emp`.`sal` > 6000.00))
1 row in set (0.00 sec)
```
