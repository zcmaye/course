...
`CHANGE REPLICATION SOURCE TO`更多选项请参考官方文档：[CHANGE REPLICATION SOURCE TO Statement](https://dev.mysql.com/doc/refman/8.4/en/change-replication-source-to.html)
...
CHANGE REPLICATION SOURCE TO
 ...
 SOURCE_USER='slave1',
 SOURCE_PASSWORD='123456',
 SOURCE_LOG_FILE='hdy-bin.000012',
 SOURCE_LOG_POS=127;
```
...
[9-同步故障处理1_哔哩哔哩_bilibili](https://www.bilibili.com/video/BV1BZ421T7Z8?spm_id_from=333.788.videopod.episodes&vd_source=d798f4dc7fdb9ca6a80baf9f4d394acb&p=9)

### 4.1 在从库上找不到主库删除的记录

在主库中插入一条记录，然后手动在从库中删掉，最后再从主库中删除，此时主从数据就不一致了！
...
```sql
INSERT INTO emp(ename,job,hiredate,sal,deptno) VALUES('hdy','sales',NOW(),5000,40);
```
...
*************************** 1. row ***************************
             Replica_IO_State: Waiting for source to send event
					...
                   Last_Errno: 1032
                   Last_Error: Coordinator stopped because there were error(s) in the worker(s). The most recent failure being: Worker 1 failed executing transaction 'ANONYMOUS' at source log 
hdy-bin.000012, end_log_pos 1170. See error log and/or performance_schema.replication_applier_status_by_worker table for more details about this failure or others, if any.
                 Skip_Counter: 0
          Exec_Source_Log_Pos: 868
...
Source_SSL_Verify_Server_Cert: No
                ...
                Last_IO_Error:
               Last_SQL_Errno: 1032
               Last_SQL_Error: Coordinator stopped because there were error(s) in the worker(s). The most recent failure being: Worker 1 failed executing transaction 'ANONYMOUS' at source log 
hdy-bin.000012, end_log_pos 1170. See error log and/or performance_schema.replication_applier_status_by_worker table for more details about this failure or others, if any.
  Replicate_Ignore_Server_Ids:
				...
1 row in set (0.00 sec)
...
*************************** 1. row ***************************
  ...
  LAST_ERROR_MESSAGE: Worker 1 failed executing transaction 'ANONYMOUS' at source log hdy-bin.000012, end_log_pos 1170; Could not execute Delete_rows event on table hdy_master_slave.emp; Can't find record in 'emp', Error_code: 1032; handler error HA_ERR_KEY_NOT_FOUND; the event's source log hdy-bin.000012, end_log_pos 1170
LAST_ERROR_TIMESTAMP: 2026-01-22 13:47:10.668779
...
1. 在binlog日志文件`hdy-bin.000012`的**1170**位置执行匿名(ANONYMOUS)事务失败；
2. 无法在表`emp`上执行`DELETE`操作，因为在`emp`表中找不到指定的记录。

因为在前面我们已经把从库中的`empno`为3的员工删除了，所以同步时就找不到`empno`为3的记录，所以报错了！
...
+ 然后设置变量跳过当前错误（这个计数器就是个“一次性消耗品”，用完后自动归零。）
...
*************************** 1. row ***************************
             Replica_IO_State: Waiting for source to send event
                ...
                Connect_Retry: 60
              Source_Log_File: hdy-bin.000012
          Read_Source_Log_Pos: 1201
               Relay_Log_File: hdy-relay.000003
                Relay_Log_Pos: 326
        Relay_Source_Log_File: hdy-bin.000012
           Replica_IO_Running: Yes
          Replica_SQL_Running: Yes
              Replicate_Do_DB:
          Replicate_Ignore_DB:
...
Source_SSL_Verify_Server_Cert: No
  ...
  Replicate_Ignore_Server_Ids:
             Source_Server_Id: 1
                  Source_UUID: 0061c3ce-ed2d-11f0-b3a4-080027d238ad
             Source_Info_File: mysql.slave_master_info
                    SQL_Delay: 0
          SQL_Remaining_Delay: NULL
    Replica_SQL_Running_State: Replica has read all relay log; waiting for more updates
...
+ 最后看一下`sql_replica_skip_counter`变量值，已经自动归0了

```sql
mysql> SHOW VARIABLES LIKE 'sql_repli%';     
+--------------------------+-------+
| Variable_name            | Value |
+--------------------------+-------+
...
> 还有一个`replica_skip_errors`变量，用来跳过指定的错误：
>
> ```
> SET PERSIST replica_skip_errors = '1062';			#开启跳过主键冲突错误(1062)
> SET PERSIST replica_skip_errors = '1062,1032';
> ```

### 4.2 主键重复

先在从机上插入一条记录，然后在主机上插入同样的记录(保证主键是相同的)！
...
```sql
INSERT INTO emp(empno,ename,job,hiredate,sal,deptno) VALUES(3,'hdy','sales',NOW(),5000,40);
```
...
*************************** 1. row ***************************
             Replica_IO_State: Waiting for source to send event
                   ...
                   Last_Errno: 1062
                   Last_Error: Coordinator stopped because there were error(s) in the worker(s). The most recent failure being: Worker 1 failed executing transaction 'ANONYMOUS' at source log 
hdy-bin.000012, end_log_pos 1511. See error log and/or performance_schema.replication_applier_status_by_worker table for more details about this failure or others, if any.
                 Skip_Counter: 0
          Exec_Source_Log_Pos: 1201
              Relay_Log_Space: 2128
              Until_Condition: None
               Until_Log_File:
                Until_Log_Pos: 0
           Source_SSL_Allowed: No
...
*************************** 1. row ***************************
  ...
  LAST_ERROR_MESSAGE: Worker 1 failed executing transaction 'ANONYMOUS' at source log hdy-bin.000012, end_log_pos 1511; Could not execute Write_rows event on table hdy_master_slave.emp; Duplicate entry '3' for key 'emp.PRIMARY', Error_code: 1062; handler error HA_ERR_FOUND_DUPP_KEY; the event's source log hdy-bin.000012, end_log_pos 1511
LAST_ERROR_TIMESTAMP: 2026-01-22 14:35:10.259675
1 row in set (0.00 sec)
```

这个错误告诉我们：

1. 在binlog日志文件`hdy-bin.000012`的**1511**位置执行匿名(ANONYMOUS)事务失败；
2. 无法在表`emp`上执行`INSERT`操作，因为在`emp`表中主键为`3`的存在冲突。

因为在前面我们已经在从库中插入了empno为3的记录，所以同步时再次插入`empno`为3的记录时，就产生了冲突。

#### 故障解决

这个故障很好解决，你可以使用上面的跳过错误的方式，也可以直接回到从库然后把重复的记录删除掉，然后就可以复制了！
...
在主库上插入一条记录，然后再从库中删除这条记录，最后在主库上跟新这条记录，此时从库中找不到对应的记录，就报错！
...
```sql
INSERT INTO emp(ename,job,hiredate,sal,deptno) VALUES('tianming','assistant',NOW(),5000,40);
```
...
```sql
UPDATE emp SET sal = 4000 WHERE empno = 4;
```
...
*************************** 1. row ***************************
             Replica_IO_State: Waiting for source to send event
                ...
                Connect_Retry: 60
              Source_Log_File: hdy-bin.000012
          Read_Source_Log_Pos: 2275
               Relay_Log_File: hdy-relay.000004
                Relay_Log_Pos: 676
        Relay_Source_Log_File: hdy-bin.000012
           Replica_IO_Running: Yes
          Replica_SQL_Running: No
  ...
  Replicate_Wild_Ignore_Table:
                   Last_Errno: 1032
                   Last_Error: Coordinator stopped because there were error(s) in the worker(s). The most recent failure being: Worker 1 failed executing transaction 'ANONYMOUS' at source log 
hdy-bin.000012, end_log_pos 2244. See error log and/or performance_schema.replication_applier_status_by_worker table for more details about this failure or others, if any.
                 Skip_Counter: 0
          Exec_Source_Log_Pos: 1892
              Relay_Log_Space: 1773
              Until_Condition: None
               Until_Log_File:
                Until_Log_Pos: 0
           Source_SSL_Allowed: No
  ...
  Replicate_Ignore_Server_Ids:
    ...
    Replica_SQL_Running_State:
           Source_Retry_Count: 10
                  Source_Bind:
      Last_IO_Error_Timestamp:
     Last_SQL_Error_Timestamp: 260122 16:17:56
               Source_SSL_Crl:
           Source_SSL_Crlpath:
           Retrieved_Gtid_Set:
            Executed_Gtid_Set:
...
*************************** 1. row ***************************
           WORKER_ID: 1
   LAST_ERROR_NUMBER: 1032
  LAST_ERROR_MESSAGE: Worker 1 failed executing transaction 'ANONYMOUS' at source log hdy-bin.000012, end_log_pos 2244; Could not execute Update_rows event on table hdy_master_slave.emp; Can't find record in 'emp', Error_code: 1032; handler error HA_ERR_KEY_NOT_FOUND; the event's source log hdy-bin.000012, end_log_pos 2244
LAST_ERROR_TIMESTAMP: 2026-01-22 16:17:56.155234
1 row in set (0.00 sec)
```

这个和第一个故障差不多，就是在从库中找不到更新的记录！

#### 故障解决

##### 分析日志

查看主机的`binlog`日志或者查看从机的`relaylog`日志，分析下主机干了什么事情！
...
| File           | Position | Binlog_Do_DB     | Binlog_Ignore_DB | Executed_Gtid_Set |
+----------------+----------+------------------+------------------+-------------------+
| hdy-bin.000012 |     2275 | hdy_master_slave |                  |                   |
+----------------+----------+------------------+------------------+-------------------+
1 row in set (0.00 sec)
```
...
```bash
root@maye-vm-pc:~# mysqlbinlog -v "/var/lib/mysql/binlog/hdy-bin.000012" | grep -A 30 'end_log_pos 2244'
#260122 16:17:56 server id 1  end_log_pos 2244 CRC32 0xf3c8f154         Update_rows: table id 91 flags: STMT_END_F

BINLOG '
NN1xaRMBAAAATwAAAGIIAAAAAFsAAAAAAAEAEGhkeV9tYXN0ZXJfc2xhdmUAA2VtcAAIAg8PAgr2
AgIGKAAkAAcC/gEBAAID/P8AeMZ3MA==
NN1xaR8BAAAAYgAAAMQIAAAAAFsAAAAAAAEAAgAI//9IBAAIdGlhbm1pbmcJYXNzaXN0YW50NtQP
gBOIACgASAQACHRpYW5taW5nCWFzc2lzdGFudDbUD4APoAAoAFTxyPM=
'/*!*/;
### UPDATE `hdy_master_slave`.`emp`
### WHERE
###   @1=4
###   @2='tianming'
###   @3='assistant'
...
###   @8=40
# at 2244
#260122 16:17:56 server id 1  end_log_pos 2275 CRC32 0x28b4c456         Xid = 55
COMMIT/*!*/;
...
+-----------------------+------------------------------------------------+
| relay_log             | hdy-relay                                      |
| relay_log_basename    | E:\MySQL\MySQL Server 8.4\Data\hdy-relay       |
| relay_log_index       | E:\MySQL\MySQL Server 8.4\Data\hdy-relay.index |
| relay_log_purge       | ON                                             |
...
从输出结果的`Relay_Log_File`字段可以看出，当前正在使用的中继日志文件为`hdy-relay.000004`。

+ 查看中继日志内容（windows没有grep命令，查看起来比较麻烦，全部查看自己找吧）

```bash
mysqlbinlog -v "E:\MySQL\MySQL Server 8.4\Data\hdy-relay.000004"
```

从`2244`位置开始的日志内容如下，和主机中的binlog日志是一样的。

```sql
# at 755
#260122 16:17:56 server id 1  end_log_pos 2067 CRC32 0x1428fc2e         Query   thread_id=14    exec_time=0     error_code=0
SET TIMESTAMP=1769069876/*!*/;
BEGIN
/*!*/;
# at 851
#260122 16:17:56 server id 1  end_log_pos 2146 CRC32 0x3077c678         Table_map: `hdy_master_slave`.`emp` mapped to number 91
...
+ 从上面的日志的UPDATE语句中直接回复更新后的数据即可，手动编写INSERT语句：

```sql
INSERT INTO emp(empno,ename,job,mgr,hiredate,sal,comm,deptno) VALUES(4,'tianming','assistant',NULL,'2026:01:22',4000,NULL,40);
```

编写INSERT语句时，最好写上所有字段，这样比较不容易出错！
...
*************************** 1. row ***************************
             Replica_IO_State: Waiting for source to send event
              ...
              Source_Log_File: hdy-bin.000012
          Read_Source_Log_Pos: 2275
               Relay_Log_File: hdy-relay.000005
                Relay_Log_Pos: 326
        Relay_Source_Log_File: hdy-bin.000012
           Replica_IO_Running: Yes
          Replica_SQL_Running: No					#SQL 线程已经被关掉了
```

+ 然后我们在主机中从现有`emp`表创建一张新表`t_emp`
...
|     1 | maye     | teacher   | NULL | 2024-06-04 | 60000.00 | NULL |     20 |
|     2 | ZC       | teacher   | NULL | 2024-10-15 |  8000.00 | NULL |     20 |
|     3 | hdy      | sales     | NULL | 2026-01-22 |  5000.00 | NULL |     40 |
|     4 | tianming | assistant | NULL | 2026-01-22 |  4000.00 | NULL |     40 |
+-------+----------+-----------+------+------------+----------+------+--------+
4 rows in set (0.00 sec)
```
...
```sql
UPDATE t_emp SET ename = 'tm',sal = 4500 WHERE empno = 4;
FLUSH LOGS;
```

> 注意：此时从库的SQL线程是关闭的，并没有复制成功！

+ 接着，我们把从库中的中继日志删掉！（Windows直接打开目录删掉，Linux执行如下命令）

```bash
$ rm -f hdy-relay.*
```

+ 好了，我们可以在从机中开启sql_thread了

```sql
START REPLICA sql_thread;
```

+ 此时查看复制状态就有错误了(部分输出如下)
...
*************************** 1. row ***************************
             Replica_IO_State: Waiting for source to send event
                  ...
                  Source_Port: 3306
                Connect_Retry: 60
              Source_Log_File: hdy-bin.000014
          Read_Source_Log_Pos: 158
               Relay_Log_File: hdy-relay.000005
                Relay_Log_Pos: 326
        Relay_Source_Log_File: hdy-bin.000012
          ...
          Replica_SQL_Running: No
                   Last_Errno: 13117
                   Last_Error: Could not open log file
                 Skip_Counter: 0
          Exec_Source_Log_Pos: 2275
              Relay_Log_Space: 3960
        Seconds_Behind_Source: NULL
Source_SSL_Verify_Server_Cert: No
                Last_IO_Errno: 0
                Last_IO_Error:
               Last_SQL_Errno: 13117
               Last_SQL_Error: Could not open log file
```

可以看到，错误原因是`Could not open log file`找不到日志文件了！

#### 故障解决

##### 确定同步点

根据从机的复制状态可以知道同步的日志文件和位置：

+ 主机的binlog日志，显示当前读取主机的binlog日志文件和位置，显示的是主库中最新的binlog日志文件。
...
+ 中继日志对应的主库日志文件（最后同步成功的主机中的日志文件以及位置）
...
跟上上面的分析，我们需要重新从` hdy-bin.000012`日志文件的`2275`位置开始同步！
...
```sql
CHANGE REPLICATION SOURCE TO SOURCE_LOG_FILE='hdy-bin.000012',SOURCE_LOG_POS=2275;
```
...
再次查看中继日志，发现已经重新创建了，然后查看一下新的表是否同步成功！
...
|     2 | ZC    | teacher   | NULL | 2024-10-15 |  8000.00 | NULL |     20 |
|     3 | hdy   | sales     | NULL | 2026-01-22 |  5000.00 | NULL |     40 |
|     4 | tm    | assistant | NULL | 2026-01-22 |  4500.00 | NULL |     40 |
+-------+-------+-----------+------+------------+----------+------+--------+
4 rows in set (0.00 sec)
```
