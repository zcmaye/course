...
SELECT * FROM hdy.emp INTO OUTFILE '/var/lib/mysql-files/emp_1.csv' FIELDS TERMINATED BY ',' ENCLOSED BY '\"';
```
...
```mysql
mysql -uroot -p --execute="SELECT * FROM emp;" hdy > "/var/lib/mysql-files/emp_2.txt"
```
...
```mysql
mysql -uroot -p --vertical --execute="SELECT * FROM emp;" hdy > "/var/lib/mysql-files/emp_3.txt"
```
...
1. 恢复数据比较安全的做法，是<font style="color:rgb(232,105,0)">`恢复出一个备份`</font>，或者找一个从库作为<font style="color:rgb(232,105,0)">`临时库 `</font>，在这个临时库上执行这些操作，然后再将确认过的临时库的数据，恢复回主库。如果直接修改主库，可能导致对数据库的<font style="color:rgb(232,105,0)">`二次破坏 `</font>
...
如果有<font style="color:rgb(232,105,0)">`非常核心的业务  `</font>，不允许太长的恢复时间，可以考虑**搭建延迟复制的备库。**一般的主备复制结构存在的问题是，如果主库上有个表被误删了，这个命令很快也会被发给所有从库，进而导致所有从库的数据表也都一起被误删了。
